...
)
{
    ...
    uint64_t re1_begin; //记录开始时间
    uint64_t re1_end; //第一个结束时间
    uint64_t time_used_re1; // 第一个使用时间（差值）

    //为什么这几个变量要使用static修饰 ： 因为每次在main函数中调用refactor这个函数时，refactor这个函数中定义的局部变量会随着
    //调用结束而被重置，所以将这几个需要连续性变化的变量设置为静态变量 就能在程序执行时具有连续性
    static uint64_t total_re1 = 0; //循环N次的时间总和
    static uint64_t re1_min_time = 10000000000000ULL;
    static uint64_t re1_max_time = 0;
    ...
    /* ---------------------------------------------------------------------- */
    /* factor each block 主要是多块的情况  time: 8310ns - 15710ns */
    /* ---------------------------------------------------------------------- */
    ...
    if (scale > 0)
    {
        ...
        for (block = 0 ; block < nblocks ; block++)
        {
            ...
            else //缩放因子多块的情况
            {
                ...
                for (k = 0 ; k < nk ; k++) //逐列对矩阵每个块进行处理 
                {

                    /* --------------time:   5000na   ---------------------------------- */
                    /* scatter kth column of the block into workspace X */
                    /* 首先确定了当前要处理的列 oldcol，然后通过内层循环遍历该列中的非零元素。
                    对于每个非零元素，根据其对应的行索引经过一些转换（通过 Pinv 等操作）得到新的行索引 newrow，
                    并根据 newrow 的情况将元素按照一定比例（通过 SCALE_DIV_ASSIGN 函数）分配到不同的数据结构中，
                    要么是 Offx（可能是处理非对角部分元素），要么是 X（可能是与当前块处理相关的工作区数据结构） */

                    oldcol = Q [k+k1] ; //确定列索引
                    pend = Ap [oldcol+1] ; //通过AP确定范围

                    for (p = Ap [oldcol] ; p < pend ; p++) //然后遍历该列的非零元素
                    {
                        oldrow = Ai [p] ;
                        newrow = Pinv [oldrow] - k1 ;
                        if (newrow < 0 && poff < nzoff)
                    ...
                    /* ------------------------------------------------------ */
                    /* 计算上三角矩阵U的第k列，并更新原始矩阵（这里记为A）的第k列。time: 11 12us*/
                    /* ------------------------------------------------------ */
                ...
                // re1_begin = rte_rdtsc();
                // re1_end = rte_rdtsc();
                // time_used_re1 = (re1_end - re1_begin) / CPU_GHZ;
                // total_re1 += time_used_re1;
                // if (time_used_re1 > re1_max_time)
                //     re1_max_time = time_used_re1;
                // if (time_used_re1 < re1_min_time)
                //     re1_min_time = time_used_re1;
                // if(idx == repeat  - 1)
                // {
                //     printf(" re1_min: %luns, re1_max: %luns, re1_avg: %luns\n",re1_min_time,re1_max_time,total_re1 / repeat );
                // }

        }

    /* ---------------------------------------------------------------------- */
    /*根据关键行顺序排列比例因子Rs 130ns - 240ns*/
    /* ---------------------------------------------------------------------- */
...
typedef struct
{
    uint64_t stat_0_2000ns;
    uint64_t stat_2000_4000ns;
    uint64_t stat_4000_6000ns;
    uint64_t stat_6000_8000ns;
    uint64_t stat_8000_10000ns;
    uint64_t stat_10000_12000ns;
    uint64_t stat_12000_14000ns;
    uint64_t stat_14000_16000ns;
    uint64_t stat_16000_18000ns;
    uint64_t stat_18000_20000ns;
    uint64_t stat_20000_22000ns;
    uint64_t stat_22000_24000ns;
    uint64_t stat_24000_30000ns;
    uint64_t stat_30000_100000ns;
    uint64_t stat_100000_1000000ns;
    uint64_t stat_over_1000000ns;
} Stat;

static Stat  factorize_stat, solve_stat;
...
int main( int argc,char **argv )
{
    ...
    for (idx  ; idx < repeat ; idx++)
    {
		...
		if (idx >= warm)
		{
			time_used_factorize = (time_factorize - time_begin) / (3.8);
			...
			if (time_used_factorize < factorize_min_time)
                ...
                if (time_used_factorize < 2000)
            {
                factorize_stat.stat_0_2000ns++;
            } 
            else if (time_used_factorize >= 2000 && time_used_factorize < 4000)
            {
                factorize_stat.stat_2000_4000ns++;
            }
            else if (time_used_factorize >= 4000 && time_used_factorize < 6000)
            {
                factorize_stat.stat_4000_6000ns++;
            }
            else if (time_used_factorize >= 6000 && time_used_factorize < 8000)
            {
                factorize_stat.stat_6000_8000ns++;
            }
            else if (time_used_factorize >= 8000 && time_used_factorize < 10000)
            {
                factorize_stat.stat_8000_10000ns++;
            }
            else if (time_used_factorize >= 10000 && time_used_factorize < 12000)
            {
                factorize_stat.stat_10000_12000ns++;
            }
            else if (time_used_factorize >= 12000 && time_used_factorize < 14000)
            {
                factorize_stat.stat_12000_14000ns++;
            }
            else if (time_used_factorize >= 14000 && time_used_factorize < 16000)
            {
                factorize_stat.stat_14000_16000ns++;
            }
            else if (time_used_factorize >= 16000 && time_used_factorize < 18000)
            {
                factorize_stat.stat_16000_18000ns++;
            }
            else if (time_used_factorize >= 18000 && time_used_factorize < 200000)
            {
                factorize_stat.stat_18000_20000ns++;
            }
            else if (time_used_factorize >= 20000 && time_used_factorize < 22000)
            {
                factorize_stat.stat_20000_22000ns++;
            }
            else if (time_used_factorize >= 22000 && time_used_factorize < 24000)
            {
                factorize_stat.stat_22000_24000ns++;
            }
            else if (time_used_factorize >= 24000 && time_used_factorize < 30000)
            {
                factorize_stat.stat_24000_30000ns++;
            }
            else if (time_used_factorize >= 30000 && time_used_factorize < 100000)
            {
                factorize_stat.stat_30000_100000ns++;
            }
            else if (time_used_factorize >= 100000 && time_used_factorize < 1000000)
            {
                factorize_stat.stat_100000_1000000ns++;
            }
            else
            {
                factorize_stat.stat_over_1000000ns++;
            }


			time_used_solve = (time_end - time_factorize) / (3.8);
			...
			if (time_used_solve < min_time)
                ...
                if (time_used_solve < 2000)
            {
                solve_stat.stat_0_2000ns++;
            } 
            else if (time_used_solve >= 2000 && time_used_solve < 4000)
            {
                solve_stat.stat_2000_4000ns++;
            }
            else if (time_used_solve >= 4000 && time_used_solve < 6000)
            {
                solve_stat.stat_4000_6000ns++;
            }
            else if (time_used_solve >= 6000 && time_used_solve < 8000)
            {
                solve_stat.stat_6000_8000ns++;
            }
            else if (time_used_solve >= 8000 && time_used_solve < 10000)
            {
                solve_stat.stat_8000_10000ns++;
            }
            else if (time_used_solve >= 10000 && time_used_solve < 12000)
            {
                solve_stat.stat_10000_12000ns++;
            }
            else if (time_used_solve >= 12000 && time_used_solve < 14000)
            {
                solve_stat.stat_12000_14000ns++;
            }
            else if (time_used_solve >= 14000 && time_used_solve < 16000)
            {
                solve_stat.stat_14000_16000ns++;
            }
            else if (time_used_solve >= 16000 && time_used_solve < 18000)
            {
                solve_stat.stat_16000_18000ns++;
            }
            else if (time_used_solve >= 18000 && time_used_solve < 200000)
            {
                solve_stat.stat_18000_20000ns++;
            }
            else if (time_used_solve >= 20000 && time_used_solve < 22000)
            {
                solve_stat.stat_20000_22000ns++;
            }
            else if (time_used_solve >= 22000 && time_used_solve < 24000)
            {
                solve_stat.stat_22000_24000ns++;
            }
            else if (time_used_solve >= 24000 && time_used_solve < 30000)
            {
                solve_stat.stat_24000_30000ns++;
            }
            else if (time_used_solve >= 30000 && time_used_solve < 100000)
            {
                solve_stat.stat_30000_100000ns++;
            }
            else if (time_used_solve >= 100000 && time_used_solve < 1000000)
            {
                solve_stat.stat_100000_1000000ns++;
            }
            else
            {
                solve_stat.stat_over_1000000ns++;
            }    
   ...
   // for (int i = 0 ; i < n ; i++) printf ("x [%d] = %g\n", i, b [i]) ;
    ...
    printf(" refactor_min: %luns, refactor_max: %luns, refactor_avg: %luns\n",factorize_min_time,factorize_max_time,total_time_factorize / repeat );
    printf("0  ~ 2000ns:  %lu\n",  factorize_stat.stat_0_2000ns);
    printf("2000 ~ 4000ns:  %lu\n", factorize_stat.stat_2000_4000ns);
    printf("4000 ~ 6000ns:  %lu\n", factorize_stat.stat_4000_6000ns);
    printf("6000 ~ 8000ns:  %lu\n", factorize_stat.stat_6000_8000ns);
    printf("8000 ~ 10000ns:  %lu\n", factorize_stat.stat_8000_10000ns);
    printf("10000 ~ 12000ns:  %lu\n", factorize_stat.stat_10000_12000ns);
    printf("12000 ~ 14000ns:  %lu\n", factorize_stat.stat_12000_14000ns);
    printf("14000 ~ 16000ns:  %lu\n", factorize_stat.stat_14000_16000ns);
    printf("16000 ~ 18000ns:  %lu\n", factorize_stat.stat_16000_18000ns);
    printf("18000 ~ 20000ns: %lu\n", factorize_stat.stat_18000_20000ns);
    printf("20000 ~ 22000ns:  %lu\n", factorize_stat.stat_20000_22000ns);
    printf("22000 ~ 24000ns:  %lu\n", factorize_stat.stat_22000_24000ns);
    printf("24000 ~ 30000ns:  %lu\n", factorize_stat.stat_24000_30000ns);
    printf("30000 ~ 100000ns:  %lu\n", factorize_stat.stat_30000_100000ns);
    printf("100000 ~ 1000000ns: %lu\n", factorize_stat.stat_100000_1000000ns);
    printf("   > 1000000ns: %lu\n", factorize_stat.stat_over_1000000ns);
    printf("\n\n");



    printf(" solve_min: %luns, solve_max: %luns, solve_avg: %luns\n",min_time,max_time,total_time_solve / repeat );
    printf("0  ~ 2000ns:  %lu\n",  solve_stat.stat_0_2000ns);
    printf("2000 ~ 4000ns:  %lu\n", solve_stat.stat_2000_4000ns);
    printf("4000 ~ 6000ns:  %lu\n", solve_stat.stat_4000_6000ns);
    printf("6000 ~ 8000ns:  %lu\n", solve_stat.stat_6000_8000ns);
