...
4.klu_analyze()函数基于klu_defaults()函数设定的参数，并根据相关的排序算法、排序策略分析分析矩阵A的结构
5.klu_factor()函数基于分析的矩阵结构结果进行缩放因子缩放等一系列操作将原系数矩阵A分解为A=LU的上下三角矩阵形式
6.klu_solve()函数基于分解的矩阵结构、分解的上下三角矩阵形式通过向前向后迭代求解原理更新X的值并组合为最终的矩阵方程的解
7.若稀疏矩阵A结构不变，只是某些非零数值发生变化，则可通过klu_refactor()函数对分解的LU矩阵进行更新，然后再进行klu_solve的求解
